...
function Controller(UsergridClass) {
  ...
  this.list = function(req, res, cb) {
    ...
    UsergridClass.findBy(criteria, limit, function(err, reply) {
      self.onSuccess(err, req, res, reply, function() {
        if (cb && cb.name !== 'callbacks' && cb.name !== 'next') { return cb(err, reply); }
        res.json(reply);
      });
    });
  };
  ...
  this.getParam = function(req, name) {
    return req.swagger ? req.swagger.params[name].value : req.params[name];
  }
}

  ...
  request(callOptions, function (err, r, data) {
    ...
    if(r.statusCode === 200) {
      ...
      if ((r.error === 'auth_expired_session_token') ||
        ...
        if (data.statusCode === 401 && r.error === 'service_resource_not_found') { // correct usergrid's return value
          data.statusCode = 404;
        }
        r.body = r.body || {};
        var error = r.body.error;
...
// changed to allow save (PUT) by name - not just UUID (use getEntityId())
